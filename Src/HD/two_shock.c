...
/*  ---------------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    if ((state->flag[i] & FLAG_HLL) || (state->flag[i+1] & FLAG_HLL)){        
      ...
      Flux (state->uL, state->vL, a2L, fL, pL, pcrL, i, i); // NEW
      Flux (state->uR, state->vR, a2R, fR, pR, pcrR, i, i); // NEW
...
/*            End iterating           */
  ...
  /* -- compute flux -- */
    ...
    Flux (us, ws, &scrh2 - i, state->flux, state->press, state->presscr, i, i); // NEW
    cstar = sqrt(scrh2);
