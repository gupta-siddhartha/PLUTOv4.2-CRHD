...
#define ADD_CRD (CR_DIFFUSION == SUPER_TIME_STEPPING)     // NEW          
 ...
 *********************************************************************** */
{
  ...
  if (C_dtp == NULL) {
    ...
    #if ADD_CRD // NEW 
     MakeState (&state);
     Ecr = ARRAY_3D(NX3_TOT, NX2_TOT, NX1_TOT, double);
    #endif
...
/* ------------------------------------------------
  ...
  /* ------------------------------------------------
    compute the CR energy density array if CRD is needed       // NEW
   ------------------------------------------------ */

  #if ADD_CRD  // NEW
   TOT_LOOP(k,j,i) Ecr[k][j][i] = V[PCR][k][j][i]/(g_gammacr - 1.0);
   vh     = state.vh;
   crd_flx = state.crd_flux;
  #endif
...
/* ----------------------------------------------------------------------   
  ...
  /* -------------------------------------------------------------------
     ...
     #if ADD_CRD  // NEW
     CRD_Flux (Ecr, &state, dcoeff, IBEG - 1, IEND, grid);
     #endif
  ...
  /* ---------------------------
    ...
    if (g_intStage == 1){ 
      ...
      IDOM_LOOP(i){
        ...
        #if ADD_CRD  // NEW
         C[ECR] = (dcoeff[i-1][ECR] + dcoeff[i][ECR])*inv_dl2;
         inv_dtp = dcoeff[i][ECR]*inv_dl[i]*inv_dl[i];
         max_inv_dtp[g_dir] = MAX(max_inv_dtp[g_dir], inv_dtp);
        #endif
      }
    }
  ...
  /* ---------------------------
    ...
    /* ---------------------------------------------------
      ...
      #if ADD_CRD // NEW
       #if GEOMETRY == CARTESIAN 
        du[ECR] += (crd_flx[i][ECR] - crd_flx[i-1][ECR])*dtdl;
       #else
        du[ECR] += (A[i]*crd_flx[i][ECR] - A[i-1]*crd_flx[i-1][ECR])*dtdV;
       #endif
      #endif
    }
  }}
...
/* ----------------------------------------------------------------------   
  ...
  /* -------------------------------------------------------------------
     ...
     #if ADD_CRD // NEW
     CRD_Flux (Ecr, &state, dcoeff, JBEG - 1, JEND, grid);
     #endif
  ...
  /* ---------------------------
    ...
    if (g_intStage == 1){
      ...
      JDOM_LOOP(j){
        ...
        #if ADD_CRD // NEW
         C[ECR] += (dcoeff[j-1][ECR] + dcoeff[j][ECR])*inv_dl2;

         inv_dtp = dcoeff[j][ECR]*inv_dl[j]*inv_dl[j];
         max_inv_dtp[g_dir] = MAX(max_inv_dtp[g_dir], inv_dtp);
        #endif
      }
    }
  ...
  /* ---------------------------
    ...
    /* ---------------------------------------------------
      ...
      /* ---------------------------------------------------
        CR DIFFUSION: build rhs in the x2 direction     // NEW
       --------------------------------------------------- */

      #if ADD_CRD  // NEW
       #if GEOMETRY != SPHERICAL
        du[ECR] += (crd_flx[j][ECR] - crd_flx[j-1][ECR])*dtdl;
       #else
        du[ECR] += (A[j]*crd_flx[j][ECR] - A[j-1]*crd_flx[j-1][ECR])*dtdV;
       #endif
      #endif
...
/* ----------------------------------------------------------------------   
  ...
  /* -------------------------------------------------------------------
     ...
     #if ADD_CRD // NEW
     CRD_Flux (Ecr, &state, dcoeff, KBEG - 1, KEND, grid);
     #endif
  ...
  /* ---------------------------
    ...
    if (g_intStage == 1){
      ...
      KDOM_LOOP(k){
        ...
        #if ADD_CRD // NEW
         C[ECR] += (dcoeff[k-1][ECR] + dcoeff[k][ECR])*inv_dl2;

         inv_dtp = dcoeff[k][ECR]*inv_dl[k]*inv_dl[k];
         max_inv_dtp[g_dir] = MAX(max_inv_dtp[g_dir], inv_dtp);
  ...
  /* ---------------------------
    ...
    /* ---------------------------------------------------
        CR DIFFUSION: build rhs in the x3 direction
       --------------------------------------------------- */
      ...
      #if ADD_CRD // NEW
       du[ECR] += (crd_flx[k][ECR] - crd_flx[k-1][ECR])*dtdl;
      #endif
    }
  }}
#endif
